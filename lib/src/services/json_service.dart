...
/// ```
dynamic getByPath(Json json, JsonPath path) {
  if (path.isEmpty) {
    return json;
  }
...
/// @hint this is an inplace operation!
setValueInJson(Json json, JsonPath path, dynamic value) {
  var parent = getByPath(json, path.sublist(0, path.length - 1));
  var key = path.last;
...
addElementToListOrInit(Json json, JsonPath path, dynamic value) {
  var parent = getByPath(json, path.sublist(0, path.length - 1));
  var key = path.last;
