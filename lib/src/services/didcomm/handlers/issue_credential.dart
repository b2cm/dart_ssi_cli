...
class DidcommIssueCredentialHandler extends AbstractDidcommMessageHandler {
  ...
  Future<IssueCredential?> handle(DidcommMessage message) async {
    ...
    if (myConnection == null) {
      throw DidcommServiceException("This wallet is not owner of any "
    ...
    if (oldCred.w3cCredential != '' && !force) {
      throw DidcommServiceException("A credential is already stored. Use force "
          "to overwrite it.", code: 9583904);
    }
    ...
    for(var kt in KeyType.values) {
      try {
        await wallet!.storeCredential(request.credentials!.first.toString(), '',
          oldCred.hdPath,
    ...
    if (successFullKeyType == null) {
      throw DidcommServiceException("Could not store credential. "
          " I cannot find a matching key type", code: 4827389234);
    }
